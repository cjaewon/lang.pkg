...
// QueryVocas chains the current query on the vocas edge.
func (bq *BookQuery) QueryVocas() *VocaQuery {
	query := &VocaQuery{config: bq.config}
...
//  WithVocas tells the query-builder to eager-loads the nodes that are connected to
// the "vocas" edge. The optional arguments used to configure the query builder of the edge.
func (bq *BookQuery) WithVocas(opts ...func(*VocaQuery)) *BookQuery {
	query := &VocaQuery{config: bq.config}
...
func (bq *BookQuery) sqlAll(ctx context.Context) ([]*Book, error) {
	...
	if query := bq.withVocas; query != nil {
		...
		query.withFKs = true
		query.Where(predicate.Voca(func(s *sql.Selector) {
			s.Where(sql.InValues(book.VocasColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.book_vocas
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "book_vocas" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
